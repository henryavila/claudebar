---
last_updated: 2026-06-25T00:44:17Z
active_count: 0
archived_count: 3
---
...
| When | Description |
|---|---|
| 2026-06-19T09:22:45Z | Fix(TDD): chip de upgrade mostrava `⬆ v1.5.0` com a 1.5.0 já instalada (em todas as máquinas pós-release). Causa-raiz: `update_chip` (`assets/statusline.sh:591`) renderizava `.update-available` verbatim sem comparar com `.version`; o arquivo fica obsoleto porque `runAutoUpdate` só o limpa no ramo `none` (throttle ≤24h) e nunca após aplicar um update. Fix: guard no chip — exibe só quando a versão anunciada é estritamente > instalada (compare x.y.z numérico; missing/unparseable → mostra). +`_CB_VERSION_FILE`. +9 testes em `test-update-chip.sh` (14 total). Suíte: node CLI + 42 bash verdes. Verificado contra o estado real em disco. NB: precisa de release npm p/ propagar às máquinas |
...
| 2026-06-25T00:44:17Z | Ad-hoc (approved **Option C**): GLM Coding Plan quota chips in claudebar. Poll Z.ai monitor API `GET api.z.ai/api/monitor/usage/quota/limit` (Auth: **raw `ANTHROPIC_AUTH_TOKEN`, NO `Bearer`**; `data.limits[].type==TOKENS_LIMIT → .percentage` = the 5h chip) from the **statusline render path** via a detached background fetch + cache — **NOT the daemon**. Rationale: the statusline (the consumer) only runs inside Claude Code, so the "user is on GLM" gate AND the "app is open" gate are both implicit; a daemon would keep polling GLM users every 5m even with the terminal closed. GLM detection = env test on the inherited `ANTHROPIC_BASE_URL` host (`api.z.ai` / `open.bigmodel.cn` / `dev.bigmodel.cn`) + `ANTHROPIC_AUTH_TOKEN`; non-GLM users never fetch. Cache TTL configurable: **5m default, 1m min** (`quota.refresh_interval_minutes` in config.toml); lockfile guards concurrent fetches; every render reads cache first so the bar stays fast. 7d window is absolute-only via `model-usage` (API gives no weekly cap → show prompts/tokens, not %). Scope: `assets/statusline.sh` (chip wiring + env gate), a new quota fetch/cache module, `assets/default-config.toml` (interval + toggle), `test/cli/`. Predecessor/sibling: `native-daemon-selfheal` (its daemon.js templates are NOT reused here). Next: TDD the fetch+cache module, then wire the 5h chip behind the GLM-gate. |
